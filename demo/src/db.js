...
function upsertProfile(sessionId, fields) {
	...
	if (!current) {
		const stmt = db.prepare('INSERT INTO profiles (session_id, household_size, monthly_income_sgd, housing_type, dependents, monthly_expenses_sgd, savings_sgd, cpf_employee_percent, liabilities_mortgage_sgd, risk_tolerance) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
		stmt.run(
			sessionId,
	...
	const merged = {
		...
		preferred_instruments: fields.preferred_instruments ?? current.preferred_instruments ?? null,
		investment_goal: fields.investment_goal ?? current.investment_goal ?? null
	};
	const stmt = db.prepare('UPDATE profiles SET household_size = ?, monthly_income_sgd = ?, housing_type = ?, dependents = ?, monthly_expenses_sgd = ?, savings_sgd = ?, cpf_employee_percent = ?, liabilities_mortgage_sgd = ?, risk_tolerance = ?, invest_horizon_years = ?, monthly_invest_sgd = ?, preferred_instruments = ?, investment_goal = ?, updated_at = datetime(\'now\') WHERE session_id = ?');
	stmt.run(
